...
esp_err_t LcdI2C::init()
{
    ...
    sendCommand(0x33);   // init
    sendCommand(0x32);   // 4-bit mode
    sendCommand(CMD_FUNCTION);
    sendCommand(CMD_DISPLAY);
    sendCommand(CMD_CLEAR);
    sendCommand(CMD_ENTRY);
...
void LcdI2C::i2c_init()
{
    ...
    // NE PAS réinstaller le driver si déjà installé
...
void LcdI2C::send(uint8_t value, uint8_t mode)
{
    ...
    {
        uint8_t high = static_cast<uint8_t>((value & 0xF0) | LCD_BACKLIGHT | mode);
        uint8_t low  = static_cast<uint8_t>(((value << 4) & 0xF0) | LCD_BACKLIGHT | mode);

        uint8_t data[4] = {
            static_cast<uint8_t>(high | LCD_ENABLE),
            high,
            static_cast<uint8_t>(low | LCD_ENABLE),
            low
        };
